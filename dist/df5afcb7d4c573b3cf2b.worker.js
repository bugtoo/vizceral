...
/***/ (function(module, exports, __webpack_require__) {
	...
	self.layout = function (options) {
	  // run the layout
	  var nodePositions = self.ltrTreeLayouter.layout(options);
	
	  // adjust the layout since our coordinates are center origin
	  var halfWidth = options.dimensions.width / 2;
	  var halfHeight = options.dimensions.height / 2;
	  var nodeName = void 0;
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var weightSort = function weightSort(a, b) {
	...
	(function () {
	  var LTRTreeLayouter = function () {
	    ...
	    function sortNodesByDepth(graph) {
	      ...
	      nodesSortedByDepth = nodesSortedByDepth.reduce(function (a, n) {
	        a.push(n);return a;
	      }, []);
	
	      var maxNodesPerDepth = 30;
	      for (var i = 0; i < nodesSortedByDepth.length; i++) {
	        var nodesInDepth = nodesSortedByDepth[i];
	        if (nodesInDepth.length > maxNodesPerDepth) {
	          var nodesToKeep = Math.min(nodesInDepth.length / 2 - 1, maxNodesPerDepth);
	          var newNodeDepth = nodesInDepth.splice(nodesToKeep);
	    ...
	    function sortNodesWithinDepth(nodesSortedByDepth) {
	      for (var i = 0; i < nodesSortedByDepth.length; i++) {
	        var nodesAtDepth = nodesSortedByDepth[i];
	        var newNodesAtDepth = [];
	        nodesAtDepth.sort(weightSort);
	
	        // Heaviest in the middle, lightest to the outside
	        for (var j = nodesAtDepth.length - 1; j >= 0; j--) {
	          if (j % 2) {
	    ...
	    function positionNodes(nodesSortedByDepth, dimensions) {
	      ...
	      function setPositions(column, nodesAtDepth, xDelta) {
	        ...
	        var yDelta = dimensions.height / (nodesAtDepth.length + 1);
	    ...
	    ltrTreeLayouter.prototype.layout = function (data) {
	      ...
	      var graph = new _ltrTreeGraph2.default(data.graph.nodes, data.graph.edges); // Build a simple graph object
	      graph.removeSameEdges(); // Remove edges that have same source and target
	      _acyclicFAS2.default.remove(graph); // Remove acyclic links
	      _ranker2.default.longestPathRanking(graph); // Run a longest path algorithm to build a layout baseline
	      // TODO: Rank the nodes from the dropped same edges...
	
	      _acyclicFAS2.default.restore(graph); // Restore acyclic links
	      graph.restoreSameEdges(); // Replace edges that have same source and target
	
	      _ranker2.default.normalizeRanks(graph); // Normalize node ranks to be 0++
	      if (!options.noRankPromotion) {
	        _ranker2.default.forcePrimaryRankPromotions(graph, data.entryNode); // Force all entry nodes to be first
	        _ranker2.default.forceSecondaryRankPromotions(graph, data.entryNode); // Force any leafs that are one level deep from specified entry node to not move all the way to the edge
	      }
	
	      var nodesSortedByDepth = sortNodesByDepth(graph);
	      sortNodesWithinDepth(nodesSortedByDepth);
	      var nodePositions = positionNodes(nodesSortedByDepth, data.dimensions);
	      return nodePositions;
	    };
...
/***/ (function(module, exports, __webpack_require__) {
	...
	function Graph(nodes, edges) {
	  ...
	  this._entryNodeMap = this.nodes.reduce(function (val, node) {
	    val[node.name] = true;
	    return val;
	  }, {});
	  this._incomingNodes = {};
	  ...
	  _lodash2.default.each(edges, function (edge) {
	    ...
	    _this._incomingNodes[edge.target] = _this._incomingNodes[edge.target] || {};
	    _this._incomingNodes[edge.target][edge.source] = true;
	
	    // Add the connection to the outgoing connections object
	    _this._outgoingNodes[edge.source] = _this._outgoingNodes[edge.source] || {};
	    _this._outgoingNodes[edge.source][edge.target] = true;
	
	    // Add the edge to the outgoing edges map
	    _this._outgoingEdges[edge.source] = _this._outgoingEdges[edge.source] || [];
	    _this._outgoingEdges[edge.source].push(edge);
	  });
	  ...
	  _lodash2.default.each(this._incomingNodes, function (targetNodes, sourceNodeName) {
	    var incomingNodeNames = Object.keys(targetNodes);
	    var outgoingNodeNames = Object.keys(_this._outgoingNodes[sourceNodeName] || {});
	    var incomingWithoutOutgoing = _lodash2.default.without.apply(_lodash2.default, [incomingNodeNames].concat(_toConsumableArray(outgoingNodeNames)));
	    if (incomingWithoutOutgoing.length !== 0) {
	      delete _this._entryNodeMap[sourceNodeName];
	    }
	  });
	}
	
	Graph.prototype.validateData = function (nodes, edges) {
	  var nodeMap = nodes.reduce(function (val, node) {
	    val[node.name] = node;
	    return val;
	  }, {});
	  ...
	  if (nodes.length > 1) {
	    for (var _i = nodes.length - 1; _i >= 0; _i--) {
	      if (!nodeMap[nodes[_i].name] || !nodeMap[nodes[_i].name].connected) {
	        nodes.splice(_i, 1);
	      }
	    }
	  }
	...
	Graph.prototype.removeEdge = function (edge) {
	  ...
	  if (this._outgoingEdges[edge.source]) {
	    _lodash2.default.remove(this._outgoingEdges[edge.source], function (anEdge) {
	      return anEdge.source === edge.source && anEdge.target === edge.target;
	    });
	  }
	  _lodash2.default.remove(this.edges, function (anEdge) {
	    return anEdge.source === edge.source && anEdge.target === edge.target;
	  });
	};
	...
	Graph.prototype.removeSameEdges = function () {
	  ...
	  _lodash2.default.each(this.edges, function (edge) {
	    if (edge && edge.source === edge.target) {
	      _this2.storedSameEdges.push(edge);
	      _this2.removeEdge(edge);
	    }
	  });
	};
	...
	Graph.prototype.restoreSameEdges = function () {
	  ...
	  _lodash2.default.each(this.storedSameEdges, function (edge) {
	    _this3.addEdge(edge);
	  });
	...
	Graph.prototype.getNode = function (nodeName) {
	  return _lodash2.default.find(this.nodes, ['name', nodeName]);
	};
...
/***/ (function(module, exports, __webpack_require__) {
	...
	function dfsFas(graph) {
	  ...
	  function dfs(node) {
	    if (_lodash2.default.has(visited, node.name)) {
	      return;
	    }
	    visited[node.name] = true;
	    stack[node.name] = true;
	    _lodash2.default.each(graph.outgoingEdges(node.name), function (edge) {
	      if (_lodash2.default.has(stack, edge.target)) {
	        fas.push(edge);
	      } else {
	  ...
	  _lodash2.default.each(graph.nodes, dfs);
	  return fas;
	} /**
	   *
	   *  Copyright 2016 Netflix, Inc.
	...
	function remove(graph) {
	  var fas = dfsFas(graph);
	  _lodash2.default.each(fas, function (edge) {
	    graph.reverseEdge(edge);
	  });
	}
	
	function restore(graph) {
	  _lodash2.default.each(graph.edges, function (edge) {
	    if (edge.reversed) {
	      graph.reverseEdge(edge);
	    }
	  });
...
/***/ (function(module, exports, __webpack_require__) {
	...
	function longestPathRanking(graph) {
	  ...
	  function dfs(nodeName) {
	    ...
	    if (!_lodash2.default.has(visited, nodeName)) {
	      visited[nodeName] = true;
	
	      var rank = _lodash2.default.min(_lodash2.default.map(graph.outgoingEdges(nodeName), function (edge) {
	        return dfs(edge.target) - minimumLength;
	      }));
	      if (rank === undefined) {
	        rank = 0;
	      }
	  ...
	  _lodash2.default.each(graph.entryNodes(), dfs);
	}
	...
	function forcePrimaryRankPromotions(graph) {
	  var entryNodeName = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'INTERNET';
	
	  var entryNodes = graph.entryNodes();
	  if (entryNodeName) {
	  ...
	  for (var i = 0; i < entryNodes.length; i++) {
	    var entryNode = graph.getNode(entryNodes[i]);
	    entryNode.rank = 0;
	  }
	}
	
	function forceSecondaryRankPromotions(graph) {
	  var entryNodeName = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'INTERNET';
	  ...
	  for (var i = 0; i < entryNodes.length; i++) {
	    var outgoingNodes = graph.outgoingNodes(entryNodes[i]);
	    for (var j = 0; j < outgoingNodes.length; j++) {
	      var node = graph.getNode(outgoingNodes[j]);
	      node.rank = 1;
	    }
	  }
	}
...
/***/ })
/******/ ]);
//# sourceMappingURL=df5afcb7d4c573b3cf2b.worker.js.map