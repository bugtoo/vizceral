...
function generateParticleSystem(
  ...
  for (let i = 0; i < size; i++) {
    // Position
    vertices[i * 3] = 0;
    vertices[i * 3 + 1] = customWidth
      ? connectionWidth - normalDistribution() * connectionWidth * 2
      : 1;
    vertices[i * 3 + 2] = customWidth
      ? connectionDepth - normalDistribution() * connectionDepth * 2
      : -2;
  ...
  geometry.addAttribute(
    "customColor",
    new THREE.BufferAttribute(customColors, 3)
  );
  geometry.addAttribute(
    "customOpacity",
    new THREE.BufferAttribute(customOpacities, 1)
  );
...
function copyParticleSystemState(newPs, oldPs) {
  ...
  copyArray(
    opacityAttr.array,
    oldPs.geometry.getAttribute("customOpacity").array
  );
...
function mapVolume(volume, rateMap) {
  ...
  for (
    i = 0;
    i < rateMap.length && rateMap[i + 1] && volume > rateMap[i + 1][0];
    i++
  ) {
...
function mapVolumesToReleasesPerTick(volumes, rateMap) {
  ...
  for (const volumeName in volumes) {
    ...
    if (volumes.hasOwnProperty(volumeName)) {
...
class ConnectionView extends BaseView {
  constructor(connection, maxParticles, customWidth) {
    ...
    this.connectionWidth =
      Math.min(
        this.object.source.getView().radius,
        this.object.target.getView().radius
      ) * 0.45;
    this.connectionDepth =
      Math.min(
        connection.source.getView().getDepth(),
        connection.target.getView().getDepth() / 2
      ) - 2;
    ...
    const ps = generateParticleSystem(
      this.particleSystemSize,
      this.customWidth,
      this.connectionWidth,
      this.connectionDepth,
      this.normalY
    ...
    this.interactiveLine = new THREE.Line(
      this.interactiveLineGeometry,
      this.interactiveLineMaterial
    );
  ...
  addConnectionLine() {
    ...
    // Simon: Change the color based on the highest traffic
    this.lineColor = GlobalStyles.getColorTrafficRGBA(
      this.object.volumePercentKeysSorted[0],
      false
    );
    // this.lineColor = GlobalStyles.getColorConnectionLine(this.object);
    ...
    this.connectionLineMaterial = new THREE.LineBasicMaterial({
      ...
      // Simon: Additive blending make connection transparent
    ...
    this.connectionLine.renderOrder = 0;
  ...
  updatePosition(depthOnly) {
    this.depth = this.dimmed
      ? Constants.DEPTH.dimmedConnection
      : Constants.DEPTH.normalConnection;
    ...
    if (!depthOnly) {
      ...
      const direction = new THREE.Vector3()
        ...
        .sub(start)
        .normalize();
      this.particles.quaternion.setFromUnitVectors(
  ...
  updateVolume() {
    ...
    this.releasesPerTick = mapVolumesToReleasesPerTick(
      this.object.volume,
      this.rateMap
    );
  }
  ...
  launchParticles(numberOfParticles, key, startX) {
    ...
    for (i = 0; i < numberOfParticles; i++) {
      ...
      const nextFreeParticleIndex = this.nextFreeParticleIndex(
        numberOfParticles
      );
      ...
      if (this.magnitude !== 0) {
        ...
        if (!this.curveY[j]) {
          const curve = new THREE.QuadraticBezierCurve3(
            ...
            new THREE.Vector3(middle, this.magnitude * 2, 0),
        ...
        if (this.yIndexes[j] + this.curveY[j][Math.floor(ratio * reso)]) {
          this.positionAttr.array[i + 1] =
            this.yIndexes[j] + this.curveY[j][Math.floor(ratio * reso)].y;
        }
      }
      // if (vx < middle) {
      // }
